...
class DHRobot:
    ...
    def calc_DerivatedArray(self, q, t):
        ...
        print(f'Checking first pose: {q[0]}')
        for j in range(N):
            is_linear = prismatic_joints[j]
            valid_range = joint_ranges[j]
            print(f'Valid range of Joint {j+1}: {valid_range}')
            print(f'Angles values: q1: {q[0]}, q2: {q[1]}')
            qd[0, j] = AF.calc_dist_in_range(q[1, j], q[0, j], is_linear, valid_range) / dt[0]
        print()

        # Central difference for the intermediate points
        for i in range(1, M - 1):
            print(f"Checking pose {i+1}: {q[i]}")
            for j in range(N):
                is_linear = prismatic_joints[j]
                valid_range = joint_ranges[j]
                print(f'Valid range of Joint {j+1}: {valid_range}')
                print(f'Angles values: q1: {q[i]}, q2: {q[i+1]}')
                qd[i, j] = AF.calc_dist_in_range(q[i + 1, j], q[i - 1, j], is_linear, valid_range) / (dt[i] + dt[i - 1])
            print()

            # Backward difference for the last time-stamp
        print(f"Checking last pose : {q[-1]}")
        for j in range(N):
            is_linear = prismatic_joints[j]
            valid_range = joint_ranges[j]
            print(f'Valid range of Joint {j+1}: {valid_range}')
            print(f'Angles values: q1: {q[-2]}, q0: {q[-1]}')
            qd[-1, j] = AF.calc_dist_in_range(q[-1, j], q[-2, j], is_linear, valid_range) / dt[-1]
        print()
